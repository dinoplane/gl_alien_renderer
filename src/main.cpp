...
#include <cstdio>
static void PrintClothStats(const ClothSystem *clothSystem)
{
    FILE *fp = fopen("cloth_stats.txt", "a");
    ...
    //
    double avgMsFrame = totalTime / frameCount * 1000;
    double forcePercent = clothSystem->totalForceTime / totalTime * 100.0;
    double solvePercent = clothSystem->totalSolveTime / totalTime * 100.0;

    // std::cout << "Application average ms/frame " << totalTime / frameCount * 1000 << " (" << frameCount / totalTime << " FPS)" << std::endl;
    fmt::print(fp, "{},{},{}\n", avgMsFrame, forcePercent, solvePercent);
    fclose(fp);
}
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        RenderToFrame(scene);
        {
            ...
            // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
            {
                ...
                if (ImGui::Button("Reset Cloth System"))
                {
                    sideLen = std::atoi(buf2);

                    ClothSystemParameters c;
                    c.particleCount = 100;
                    c.timeStep = 0.005;
                    c.shaderName = "cloth";
                    c.clothSideLength = sideLen;
                    ...
                    if (scene.particleSystems.size() > 0)
                    {
                        scene.particleSystems[0]->Reinitialize(&c);
                        scene.particleSystems[0]->InitializeBuffers();
                        totalTime = 0.0;
                        frameCount = 1;
                    }
                }
                if (ImGui::Button("Print Cloth Stats"))
                {
                    if (scene.particleSystems.size() > 0)
                    {
                        PrintClothStats(static_cast<ClothSystem *>(scene.particleSystems[0].get()));
                    }
                }
                ImGui::End();
            }
            ...
            //if ((totalTime > 30.0 && sideLen < 10) 
            //    || (totalTime > 60.0 && sideLen < 15) 
            //    || (totalTime > 90.0 && sideLen >= 15)) {
            //    PrintClothStats(static_cast<ClothSystem*>(scene.particleSystems[0].get()));
            //    sideLen += 1;
            //    ClothSystemParameters c;
            //    c.particleCount = 100;
...
void processInput(GLFWwindow *window, Renderer *renderer, Scene *scene)
{ // abhorrent should only pass in renderer.
